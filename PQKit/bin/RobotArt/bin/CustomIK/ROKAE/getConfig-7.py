...
path = dllPath+'\\RKCalIK\\Cal_IK7.dll'
#path="R:\\Runtime\\bin.64.Debug\\ROKAE\\Cal_IK7.dll"
so = cdll.LoadLibrary
lib = so(path)
getPsi = lib.Get_Config

getPsi.argtypes = [c_double*16,c_double * 7, c_double * 14, c_int * 8]  ##定义求臂角的函数 参数
getPsi.restype = c_double  ##函数返回值
EndPosture = (c_double * len(tmp_endPosture))(*tmp_endPosture)
Joints = (c_double*len(CurJoint))(*CurJoint)
JointMinMax = (c_double*len(JointMinMax))(*JointMinMax)
vecJointConfigType = c_int*8
vecConfig = vecJointConfigType()
##vecConfig --> cfx cf1 cf2 cf3 cf4 cf5 cf6 cf7 
lib.Get_Config(EndPosture, Joints,JointMinMax,vecConfig)
idx = Engine.GetReturnIdxvalue()
strValue = str(vecConfig[idx])
#print(strValue)
